server <- function(input, output, session) {
  ...
  observeEvent(input$tune_sample_button, {
    ...
    tune_load_ncells <- input$tune_load_ncells
    validate(need(tune_load_ncells, 'Number of cells!'))
    my_fb <- mem$my_fb
    req(my_fb)
    tune_load_ncells <- as.integer(tune_load_ncells)
  ...
  output$tune_ui_channel <- renderUI({
    ...
    req(my_fb)
    if (isolate(debugging())) browser()
    ok <- !is.na((my_fb@panel$batchnorm_method))
    channels <- my_fb@panel$antigen[ok]
  ...
  # Set the UI for batch modeling
  output$tune_ui_batch <- renderUI({
  ...
  # Set the UI for transformation
  output$tune_ui_transf <- renderUI({
  ...
  observeEvent({
    ...
    warning("Change of tune_transf_method, tune_transf_params")
    if (isolate(debugging())) browser()
    ...
    tune_channel <- input$tune_channel
    req(tune_channel)
  ...
  observeEvent({
    ...
    warning("Change of tune_batch_method, tune_batch_params, tune_batch_zero, tune_batch_transf")
    if (isolate(debugging())) browser()
  ...
  output$tune_main_plot <- renderPlot({
    ...
    idx <- guess_match_channels(my_fb_ref, channel)
    validate(need(idx, "Select a channel in the menu."))
    channel <- my_fb_ref@panel$fcs_colname[idx]
